...
class Bus {
  public:
    ...
    static constexpr size_t   getNumberOfPins(uint8_t type)     { return isUsermod(type) ? 5 : isVirtual(type) || isHub75(type) ? 4 : isPWM(type) ? numPWMPins(type) : is2Pin(type) + 1; } // credit @PaoloTK
